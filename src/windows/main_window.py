...
class MainWindow(updates.UpdateWatcher, QMainWindow):
    ...
    def _raise_project_files_dock_if_open(self):
        ...
        if not dock.toggleViewAction().isChecked():
            return
        editor_panel = getattr(self, "editorPanel", None)
        if editor_panel:
            editor_panel.set_active_section(EditorSection.ASSETS)
        dock.raise_()
        dock.activateWindow()
    ...
    def _setup_editor_panel(self):
        """Combine legacy editing docks into one rail-driven editor surface."""
        _ = get_app()._tr
        settings = get_app().get_settings()
        contents = {
            "elements": self.dockFilesContents,
            "transitions": self.dockTransitionsContents,
            "emojis": self.dockEmojisContents,
            "effects": self.dockEffectsContents,
            "properties": self.dockPropertiesContents,
        }
        filters = {
            "elements": self.filesFilter,
            "transitions": self.transitionsFilter,
            "emojis": self.emojisFilter,
            "effects": self.effectsFilter,
            "properties": self.txtPropertyFilter,
        }
        list_views = {
            "elements": self.filesListView,
            "transitions": self.transitionsListView,
            "emojis": self.emojiListView,
            "effects": self.effectsListView,
        }

        for search in filters.values():
            search.hide()
        for content in contents.values():
            content.setParent(None)

        self.editorPanel = EditorPanel(
            self, settings, _, contents, filters, list_views, self.dockFiles)
        self.dockFiles.setWindowTitle(_("Editor"))
        self.dockFiles.setWidget(self.editorPanel)

        self._editor_legacy_docks = (
            self.dockTransitions,
            self.dockEmojis,
            self.dockEffects,
            self.dockProperties,
        )
        for dock in self._editor_legacy_docks:
            dock.setWidget(QWidget(dock))
            dock.toggleViewAction().setVisible(False)
        self._enforce_editor_panel_layout()

    def _enforce_editor_panel_layout(self):
        """Prevent restored layouts from reviving the retired editing docks."""
        if not getattr(self, "editorPanel", None):
            return
        for dock in getattr(self, "_editor_legacy_docks", ()):
            self.removeDockWidget(dock)
            dock.hide()
        if self.dockWidgetArea(self.dockFiles) == Qt.NoDockWidgetArea:
    ...
    def _anchor_and_show_properties_dock(self):
        ...
        editor_panel = getattr(self, "editorPanel", None)
        if editor_panel:
            editor_panel.set_active_section(EditorSection.PROPERTIES)
            if self.dockWidgetArea(self.dockFiles) == Qt.NoDockWidgetArea:
                self.addDockWidget(Qt.LeftDockWidgetArea, self.dockFiles)
            self.dockFiles.show()
    ...
    def _view_menu_docks(self):
        ...
        editor_legacy_docks = {
            "dockEffects", "dockEmojis", "dockProperties", "dockTransitions"
        }
        docks = [
            dock for dock in self.getDocks()
            if (dock.objectName() not in scope_dock_names
                and dock.objectName() not in editor_legacy_docks
    ...
    def actionProperties_trigger(self):
        ...
        if getattr(self, "editorPanel", None):
            self._anchor_and_show_properties_dock()
            return
    ...
    def createPopupMenu(self):
        ...
        for dock in sorted(self.getDocks(), key=lambda d: d.windowTitle()):
            if dock.objectName() in editor_legacy_docks | {"dockTimeline", "dockTutorial"}:
                continue
            action = dock.toggleViewAction()
            action.setEnabled(True)
    ...
    def actionColor_Grade_View_trigger(self):
        ...
        if getattr(self, "editorPanel", None):
            self.editorPanel.set_active_section(EditorSection.PROPERTIES)
    ...
    def style_dock_widgets(self, theme_changed=False):
        ...
        for dock_widget in self.getDocks():
            ...
            elif theme and theme.name in (
                    ThemeName.COSMIC.value, ThemeName.BENTO.value):
                if tabified_widgets:
