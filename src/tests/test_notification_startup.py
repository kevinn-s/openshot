...
_dll_handle = None
_binary_path = os.environ.get("OPENSHOT_TEST_BINARY_PATH", "")
if os.name == "nt" and os.path.isdir(_binary_path):
    _dll_handle = os.add_dll_directory(_binary_path)
...
def verify():
    ...
    try:
        ...
        for theme in ("Bento", "Cosmic Dusk", "Humanity: Dark", "Retro"):
...
'''
        ...
        binding_path = os.path.join(root, "libopenshot", "python")
        binary_path = os.path.join(root, "libopenshot", "bin")
        env = dict(
            os.environ,
            PYTHONPATH=os.pathsep.join((binding_path, source)),
            PATH=os.pathsep.join((binary_path, os.environ.get("PATH", ""))),
            OPENSHOT_TEST_BINARY_PATH=binary_path,
