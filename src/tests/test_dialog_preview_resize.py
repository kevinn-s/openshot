...
class DialogPreviewResizeTests(unittest.TestCase):
    ...
    def test_player_worker_refresh_keeps_selected_file_reader(self):
        signal = DummySignal()
        worker = types.SimpleNamespace(
            parent=types.SimpleNamespace(initialized=True, LoadFileSignal=signal),
            player=types.SimpleNamespace(Mode=lambda: openshot.PLAYBACK_PAUSED, Position=lambda: 25),
            reader_mode="clip",
            _seek_lock=threading.Lock(),
            _pending_seek=None,
        ...
        self.assertEqual(signal.calls, 0)
        self.assertEqual(PlayerWorker._take_pending_seek(worker), (25, False))
